...
mod tests {
    use super::*;
    use crate::mutation::OPERATION_LOCK;
    use prismux_core::{
        AccountRef, AccountStatus, Availability, AvailabilityState, ConfigProfile, DoctorReport,
        ImportConfigOptions, ImportedConfig, LoginOptions, PlatformCapabilities, PlatformInfo,
        PlatformInstall, PlatformPlugin, PlatformPoolSummary, PrismuxError, ResetCreditOutcome,
        Result, SaveOptions, SwitchReport, UsageResetCredits, UsageSnapshot,
    };
    use std::sync::{
    ...
    fn accounts_report_maps_reset_credit_metadata() {
        ...
        status.usage.as_mut().unwrap().reset_credits = Some(UsageResetCredits {
            available_count: 2,
            credits: vec![prismux_core::UsageResetCredit {
    ...
    fn account_refresh_calls_single_account_refresh_once() {
        ...
        assert_eq!(report.operation.message, "Account quota refreshed.");
        assert_eq!(refresh_count.load(Ordering::SeqCst), 1);
    }
    ...
    impl PlatformPlugin for FakePlugin {
        ...
        fn list_accounts(&self) -> Result<Vec<AccountStatus>> {
            if let Some(message) = self.list_error.as_ref() {
                return Err(PrismuxError::Message(message.clone()));
            }
        ...
        fn consume_reset_credit(
            ...
            self.reset_outcome.clone().ok_or_else(|| {
                PrismuxError::Message("reset credit outcome not configured".to_string())
            })
        }
    ...
    fn account_for_provider(
        ...
        AccountStatus {
            ...
            usage: Some(UsageSnapshot {
                source: prismux_core::UsageSource::StoredSnapshot,
                ...
                diagnostics: diagnostic
                    .map(|message| {
                        vec![prismux_core::UsageDiagnostic {
                            code: "test".to_string(),
