...
pub struct ProviderRuntimeView {
    ...
    pub source_strategy: Vec<UsageSourceStrategy>,
}
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct UsageSourceStrategy {
    pub id: String,
    pub label: String,
    pub priority: u8,
    pub foreground_eligible: bool,
...
pub fn provider_runtime_view(provider: &str, now: u64) -> ProviderRuntimeView {
    ...
    ProviderRuntimeView {
        ...
        source_strategy: usage_source_strategy(provider),
    }
}

pub fn usage_source_strategy(provider: &str) -> Vec<UsageSourceStrategy> {
    vec![
        UsageSourceStrategy {
            id: "local_usage".to_string(),
            label: format!("{provider} local usage"),
            priority: 10,
        ...
        UsageSourceStrategy {
            id: "provider_remote".to_string(),
            label: format!("{provider} provider API"),
            priority: 20,
            foreground_eligible: true,
            background_eligible: false,
            timeout_seconds: DEFAULT_REFRESH_TIMEOUT_SECONDS,
            fallback_reason: Some(
                "remote provider sources are foreground-only until explicitly enabled".to_string(),
            ),
...
mod tests {
    ...
    #[test]
    fn source_strategy_marks_remote_source_foreground_only() {
        let strategy = usage_source_strategy("codex");
        let remote = strategy
            .iter()
            .find(|source| source.id == "provider_remote")
            .unwrap();

        assert!(remote.foreground_eligible);
        assert!(!remote.background_eligible);
        assert_eq!(remote.confidence, SourceConfidence::High);
    }
