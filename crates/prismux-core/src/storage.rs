...
pub fn state_root() -> Result<PathBuf> {
    if let Some(path) = env::var_os("PRISMUX_STATE_ROOT").filter(|value| !value.is_empty()) {
        return Ok(PathBuf::from(path));
    }

    data_local_dir()
        .map(|path| path.join("prismux"))
        .ok_or_else(|| PrismuxError::Message("could not resolve the Prismux data directory".into()))
}

pub fn io_error(path: &Path, err: io::Error) -> PrismuxError {
    PrismuxError::Message(format!("{}: {err}", display_path(path)))
}
