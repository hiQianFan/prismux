...
OpenMux 的核心仍是 account/profile switch。Usage 必须服务于快速判断，不能使 CLI 演化为独立 analytics 产品。
...
- 为 account/profile attribution 定义“本变更不得伪归因”的安全边界。
...
- 不在本变更中承诺跨平台桌面应用或团队 usage 服务。
- 不维护 OpenMux 自有的全量模型 pricing 数据库。
- 不按当前 active account 反推历史 event 的 account/profile 归属，也不交付 account group-by。
...
omx usage [client]
omx usage --group-by client|day|model
omx usage --details
omx usage --no-scan
omx usage --json
```
...
- `--group-by model` 借鉴 tokscale Models lens 和 ccusage model breakdown。
...
human table 和 JSON 都从同一个 query/result model 派生，但不要求 table 镜像 JSON 的全部字段。Schema 升级遵循 additive-first；删除或重命名字段需要提升 major `schema_version`。
...
### 5. Account/profile attribution 本次只做防误归因

token usage 与 subscription quota 是独立数据。Local session logs 通常不能可靠说明某条历史 event 使用了哪个 OpenMux account；因此本变更只固定负向规则：

- 不允许把扫描时的 current account 应用到历史 event；
- 第一阶段 CLI 不提供 account group-by；
- 如果现有字段没有 attribution 证据，查询层必须保持 `unknown` 或不展示账号维度；
- attribution evidence、coverage 门槛和账号统计进入独立 proposal。

这保留 OpenMux 的潜在差异化能力，同时避免本变更引入半成品 attribution schema。

### 6. CLI table 与 JSON 共用 query service，不复制 presentation
...
StateStore 负责 SQL，application service 负责扫描编排和 report 组装，CLI 只负责参数映射与渲染。
...
- [account attribution 被误解为精确] → 本变更不交付 account group-by，只增加“不得使用 current account 反推历史”的回归。
...
5. 保留 OpenMux-owned JSON contract，供 CLI、测试和未来本地 UI 独立复用。

回滚策略：CLI 可暂时保留 legacy detailed renderer，通过单独函数切回；数据 schema 和历史 events 不回滚、不删除。
