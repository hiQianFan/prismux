...
外观立场介于“系统工具面板”与“轻品牌产品”之间：深色模式下 popover 带一层很淡的靛紫品牌底色，provider 用各自官方品牌色做强调，其余一律走 Apple HIG——系统字体、系统动态色、SF Symbols、原生 popover 行为、动态深浅色、Reduce Motion。macOS 26/27 的 Liquid Glass 由系统材料和原生控件承接，不手搓玻璃 shader，不做多层透明卡片叠加。除非系统控件无法表达状态，否则不自造主题系统。
...
- `body`：行主标签、正文。
- `label`：`caption` + secondary，metadata、时间、empty 文案。
...
4. **Footer**：freshness/CLI 状态串、`...` 溢出菜单、Quit。
...
SF Symbol `triangle`，默认不展示账号、token、raw account id 或 auth payload。Settings 提供 `icon_only` 模式隐藏文字；icon-only 时仍保留 tooltip “Prismux”。点击只 toggle popover，不触发 foreground interactive refresh；后台 timer 和手动 Refresh 才刷新数据。

### Header

左侧 provider 图标块（`triangle`，stale 时换警告 symbol）+ “Prismux” + 刷新状态副标题（`Updated 3:42 PM` / stale 时橙色 `Stale …`）。右侧两个 icon button：Refresh（`arrow.clockwise`，仅用户显式点击或目标 refresh 时换 `ProgressView` 并 disable）、Settings（`gearshape`）。无害 skip（例如 already fresh）不占主内容区，最多体现在 header freshness。icon button 用 borderless pressable chrome，必须有 `.help()` 和 accessibility label。
...
横向分页容器，按选中 index 平移（280ms smooth，方向由 index delta 自然得出）。每页独立 `ScrollView`。Overview 页聚焦 Providers card（每行一个 provider 摘要，可点进）。Provider 页聚焦 Accounts card 与 Profiles card。Menubar 主界面不展示 `Needs attention` 或 `Diagnostics` 卡；排障信息留给当下错误、Settings/About support report 和 CLI/doctor。
...
每行包含：display label、secondary label、active marker、quota/status 摘要、操作区。Active row 用 accent 10% 背景。**Switch 成功前不移动 active marker**；失败保留原 active。操作区固定宽度。Account row 额外支持 refresh、reset usage limit（Codex）、delete；delete 与 reset 走 inline 二次确认，不是默认高频按钮。target 级诊断在额度条下方显示为一条常驻诊断行（severity 图标 + 人话 message，不显示 snake_case code，recovery 进 tooltip）；不做展开/折叠，也不放操作按钮（操作走 overflow 菜单与 Accounts 卡入口）。列表用 stable identity（`localId` / target key），刷新或排序不重建整列。
...
Menubar 主内容区不显示 operation banner。成功、skip、fresh enough 等常规结果静默，header/footer freshness 承担状态反馈；失败仍通过对应行状态、错误页、Settings/About support report 或 CLI/doctor 排障。Overview 不展示 `Needs attention`，Provider 页不展示 `Diagnostics`，也不展示空诊断状态。target 诊断用两档 symbol：auth/401/403 用 octagon + failed 色，其余用 triangle + warning 色。

### Motion

遵守 Apple HIG：动效解释状态变化，不做装饰。时长 120–280ms，ease-out / 系统 spring，无弹跳过冲循环。只给明确状态值绑定动画（selected provider、refreshing/switching/deleting id、active marker），不对 root view 用无差别 `.animation`。所有 custom transition 在 `accessibilityReduceMotion` 为真时退化为 `.opacity` 或 `.identity`。Popover 用 `NSPopover` 原生开合，不自造窗口动画。
...
- 显示 last-good 数据并标 stale；打开 popover 不重复 foreground refresh。
- target 诊断只进对应账号/profile 行的一条常驻诊断行，不做展开/折叠，不做全局诊断卡。
...
- 不在 Overview/Provider 页放 `Needs attention` / `Diagnostics` 主卡片。
