...
use prismux_core::{
    ...
    ImportConfigOptions, ImportedConfig, LOGIN_TIMEOUT, LoginOptions, PlatformCapabilities,
    PlatformInfo, PlatformInstall, PlatformPlugin, PlatformPoolSummary, PrismuxError,
...
impl Default for CodexPlugin {
    fn default() -> Self {
        Self {
            codex_home: None,
            state_root: None,
            codex_executable: env::var_os("PRISMUX_CODEX_BIN")
...
impl CodexPlugin {
    ...
    fn fetch_codex_reset_credit_consume(
        ...
        .map_err(|err| PrismuxError::Message(format!("encode reset credit request: {err}")))?;
        let payload = self
    ...
    fn sync_active_auth_snapshot(
        ...
        let (Some(current_subject), Some(stored_subject)) = (current_subject, stored_subject)
            ...
            return Err(PrismuxError::Message(format!(
                "active Codex auth changed, but its account identity cannot be verified against account #{}; run `prismux save codex` or log in again before switching",
                active.display_number
            )));
        };
        if current_subject != stored_subject {
            return Err(PrismuxError::Message(format!(
                "active Codex auth does not belong to Prismux account #{}; refusing to overwrite its stored snapshot",
                active.display_number
            )));
        }
    ...
    fn deactivate_managed_model_provider(&self) -> Result<Option<LiveConfigMutation>> {
        if !self
            ...
            .is_some_and(|provider| provider.starts_with("prismux-"))
        {
            return Ok(None);
        }
        self.mutate_live_config(|document| {
            ...
            (managed_provider_id.as_deref(), expected_provider_item)
        {
            self.mutate_live_config(|document| {
                    ...
                    .and_then(Item::as_table_mut)
                {
                    ...
                    {
                        return Err(PrismuxError::Message(format!(
                            "Codex provider `{provider_id}` was modified outside Prismux; refusing to remove it"
                        )));
                    }
...
impl PlatformPlugin for CodexPlugin {
    ...
    fn refresh_account(&self, selector: &str) -> Result<AccountStatus> {
        ...
        let account = store
            .account_by_selector(self.id(), selector)?
            .ok_or_else(|| {
                PrismuxError::Message(format!("`{selector}` did not match any Codex account"))
            })?;
    ...
    fn login(&self, options: LoginOptions) -> Result<AccountRef> {
        ...
        if same_active_account {
            ...
            if let Err(err) = activation_result {
                ...
                return match rollback {
                    Ok(_) => Err(PrismuxError::Message(format!(
    ...
    fn use_target(&self, selector: &str) -> Result<UseReport> {
        ...
        match (account_match, profile_match) {
            (Some(account), Some(profile)) => Err(PrismuxError::Message(format!(
            ...
            (None, None) => Err(PrismuxError::AccountNotFound {
                platform: self.id().to_string(),
                account: selector.to_string(),
            }),
        }
...
fn ensure_model_providers_table(document: &mut DocumentMut) -> Result<()> {
    match document.get("model_providers") {
        Some(item) if !item.is_table() => Err(PrismuxError::Message(
...
fn parse_codex_toml_import(
    ...
    let value: toml::Value = toml::from_str(content).map_err(|err| {
        PrismuxError::Message(format!("invalid Codex TOML config fragment: {err}"))
    })?;
...
fn parse_codex_kv_import(
    ...
    let base_url = find_var(&vars, &["OPENAI_BASE_URL", "OPENAI_API_BASE", "BASE_URL"])
        .ok_or_else(|| {
            PrismuxError::Message("Codex KV import needs OPENAI_BASE_URL or OPENAI_API_BASE".into())
        })?;
...
fn usage_diagnostic_from_error(error: PrismuxError) -> UsageDiagnostic {
    UsageDiagnostic {
...
fn parse_reset_credit_outcome(payload: &serde_json::Value) -> Result<ResetCreditOutcome> {
    match payload.pointer("/code").and_then(serde_json::Value::as_str) {
        ...
        Some(other) => Err(PrismuxError::Message(format!(
            "unknown Codex reset credit response code: {other}"
        ))),
        None => Err(PrismuxError::Message(
