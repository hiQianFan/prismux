use prismux_core::{
    ...
    PrismuxError, ProfileRecord, RemoveReport, RemovedAccount, RemovedConfig, Result, SaveOptions,
    StateStore, SwitchReport, TargetCatalog, TargetKind, TargetResolution, UpsertAccount,
    UpsertProfile, UseReport, platform_info, run_cancellable_login,
    storage::{
...
impl Default for ClaudePlugin {
    fn default() -> Self {
        Self {
            claude_home: None,
            state_root: None,
            claude_executable: env::var_os("PRISMUX_CLAUDE_BIN")
...
impl ClaudePlugin {
    ...
    fn claude_home(&self) -> Result<PathBuf> {
        ...
        home_dir()
            ...
            .ok_or_else(|| PrismuxError::Message("could not resolve the home directory".into()))
    }
    ...
    fn import_profile(&self, options: ImportConfigOptions) -> Result<ImportedConfig> {
        ...
        let snapshot_bytes = serde_json::to_vec_pretty(&snapshot).map_err(|err| {
            PrismuxError::Message(format!("failed to encode Claude profile: {err}"))
        })?;
    ...
    fn use_profile(&self, selector: &str) -> Result<ConfigSwitchReport> {
        ...
        let snapshot: ProfileSnapshot = serde_json::from_slice(&snapshot_bytes).map_err(|err| {
            PrismuxError::Message(format!("invalid Claude profile snapshot: {err}"))
        })?;
        ...
        let next_bytes = serde_json::to_vec_pretty(&settings).map_err(|err| {
            PrismuxError::Message(format!("failed to encode Claude settings: {err}"))
        })?;
        ...
        if let Err(err) = self.state_store()?.set_active_profile(
            ...
            return match rollback {
                Ok(()) => Err(PrismuxError::Message(format!(
                    "failed to update state store after applying profile; settings were rolled back: {err}"
                ))),
                Err(rollback_err) => Err(PrismuxError::Message(format!(
...
impl ClaudePlugin {
    ...
    fn credential_backend(&self) -> Result<Box<dyn CredentialBackend>> {
        match &self.credential_backend {
            ...
            None if self.claude_home.is_none() && env::var_os("CLAUDE_CONFIG_DIR").is_none() => {
                let service = env::var("PRISMUX_CLAUDE_KEYCHAIN_SERVICE")
                    .unwrap_or_else(|_| "Claude Code".to_string());
                let account = env::var("PRISMUX_CLAUDE_KEYCHAIN_ACCOUNT")
    ...
    fn import_account(&self, name: Option<String>) -> Result<AccountRef> {
        ...
        if !backend.exists() {
            return Err(PrismuxError::PlatformNotDetected(format!(
...
impl PlatformPlugin for ClaudePlugin {
    ...
    fn switch_to(&self, selector: &str) -> Result<SwitchReport> {
        ...
        let oauth_value: Value = serde_json::from_slice(&oauth_account).map_err(|err| {
            PrismuxError::Message(format!("invalid stored oauthAccount metadata: {err}"))
        })?;
        let backend = self.credential_backend()?;
        let settings_path = self.settings_path()?;
    ...
    fn doctor(&self) -> Result<DoctorReport> {
        let backend = self.credential_backend()?;
        let settings_path = self.settings_path()?;
        let state_path = self.state_root()?.join("prismux.sqlite");
...
fn parse_credentials(bytes: &[u8]) -> Result<ParsedCredentials> {
    let value: Value = serde_json::from_slice(bytes)
        .map_err(|err| PrismuxError::Message(format!("invalid Claude credentials JSON: {err}")))?;
    let oauth = value
        .get("claudeAiOauth")
        .or_else(|| value.pointer("/secureStorage/claudeAiOauth"))
        .ok_or_else(|| {
            PrismuxError::Message("Claude credentials do not contain claudeAiOauth".into())
        })?;
...
fn parse_profile(content: &str, requested_name: Option<&str>) -> Result<ParsedProfile> {
    ...
    let explicit_profile_name = vars
        .iter()
        .find(|(key, value)| {
            (key == "PRISMUX_PROFILE" || key == "name" || key == "NAME") && !value.is_empty()
        })
...
fn parse_json_vars(content: &str) -> Result<Vec<(String, String)>> {
    let value: Value = serde_json::from_str(content)
        .map_err(|err| PrismuxError::Message(format!("invalid Claude JSON profile: {err}")))?;
    let object = value
        .as_object()
        .ok_or_else(|| PrismuxError::Message("Claude JSON profile must be an object".into()))?;
    Ok(object
        .iter()
...
fn parse_toml_vars(content: &str) -> Result<Vec<(String, String)>> {
    let value: toml::Value = toml::from_str(content)
        .map_err(|err| PrismuxError::Message(format!("invalid Claude TOML profile: {err}")))?;
...
fn resolve_profile_name(
    ...
    if name.is_empty() {
        return Err(PrismuxError::Message("profile name cannot be empty".into()));
    }
    Ok(name)
}
...
fn parse_settings(bytes: &[u8]) -> Result<Value> {
    serde_json::from_slice(bytes)
        .map_err(|err| PrismuxError::Message(format!("invalid Claude settings JSON: {err}")))
}
...
#[cfg(target_os = "macos")]
mod macos_keychain {
    use prismux_core::{PrismuxError, Result};
    use std::{
        ffi::{c_char, c_void},
        ptr, slice,
    };
    ...
    pub(super) fn delete(service: &str, account: &str) -> Result<()> {
        match find_item(service, account) {
            ...
            Err(PrismuxError::Message(message)) if message.contains("not found") => Ok(()),
            Err(err) => Err(err),
        }
    }
    ...
    fn checked_len(len: usize, label: &str) -> Result<u32> {
        u32::try_from(len)
            .map_err(|_| PrismuxError::Message(format!("{label} is too large for Keychain API")))
    }

    fn keychain_error(action: &str, status: i32) -> PrismuxError {
        PrismuxError::Message(format!("failed to {action}: OSStatus {status}"))
    }
