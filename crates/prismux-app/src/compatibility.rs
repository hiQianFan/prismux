...
fn backend_runtime_options() -> Vec<BackendRuntimeOption> {
    vec![
        BackendRuntimeOption {
            ...
            supported: true,
            guidance: "Preferred for Menubar: call prismux-menubar-ffi directly and gate by schema before mutations."
                .to_string(),
        },
        BackendRuntimeOption {
        ...
        BackendRuntimeOption {
            ...
            supported: true,
            guidance: "Allowed fallback: invoke installed prismux only for documented machine-readable commands."
                .to_string(),
        },
    ]
}
...
fn install_guidance(module: &str) -> String {
    match module {
        "cli" => "Install the prismux CLI or use a full bundle before running advanced account management commands.",
        "menubar" => "Install Prismux.app or use the CLI-only workflow for account management.",
...
mod tests {
    ...
    fn unavailable_optional_modules_keep_contract_compatible_with_guidance() {
        ...
        assert!(
            cli.install_guidance
                .as_deref()
                .unwrap()
                .contains("prismux CLI")
        );
        assert_eq!(
            result.diagnostics[0].code,
