...
pub fn run_cancellable_login(
    ...
    let mut child = command
        .spawn()
        .map_err(|err| PrismuxError::Message(format!("failed to run {describe} login: {err}")))?;
    ...
    loop {
        ...
        if cancel_requested() {
            ...
            let _ = child.wait();
            reset_login_cancel();
            return Err(PrismuxError::Message("login cancelled".to_string()));
        }
...
mod tests {
    ...
    fn cancel_kills_a_long_running_child() {
        ...
        match waiter.join().unwrap() {
            Err(PrismuxError::Message(msg)) => assert_eq!(msg, "login cancelled"),
    ...
    fn timeout_kills_a_long_running_child() {
        ...
        match result {
            Err(PrismuxError::Message(msg)) => assert!(msg.contains("timed out"), "{msg}"),
