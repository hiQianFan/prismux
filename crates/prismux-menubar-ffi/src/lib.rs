...
#[unsafe(no_mangle)]
pub extern "C" fn prismux_menubar_call(request_json: *const c_char) -> *mut c_char {
...
/// # Safety
///
/// `value` must be a pointer returned by `prismux_menubar_call` and must be freed at most once.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn prismux_menubar_free(value: *mut c_char) {
    if value.is_null() {
        return;
    }
    unsafe {
...
fn call_request(request_json: &str) -> String {
    ...
    match dispatch(request) {
        Ok(success) => encode(ResponseEnvelope {
            ...
            control_plane_schema_version: prismux_app::compatibility::CONTROL_PLANE_SCHEMA_VERSION,
            state_schema_version: prismux_app::compatibility::STATE_SCHEMA_VERSION,
            minimum_backend_version: prismux_app::compatibility::MIN_BACKEND_VERSION.to_string(),
            minimum_frontend_version: prismux_app::compatibility::MIN_FRONTEND_VERSION.to_string(),
            ok: true,
...
fn dispatch(request: RequestEnvelope) -> Result<DispatchSuccess, (&'static str, String)> {
    ...
    match request.op.as_str() {
        "accounts" => {
            ...
            json_value(prismux_app::menubar_accounts(&plugins, query)).map(DispatchSuccess::fresh)
        }
        "dashboard" => {
            let query: DashboardQuery = payload_or_default(request.payload)?;
        ...
        "cancel_login" => {
            ...
            prismux_core::request_login_cancel();
...
fn snapshot_path() -> prismux_core::Result<std::path::PathBuf> {
    Ok(state_root()?
        .join("control-plane")
...
fn encode(response: ResponseEnvelope) -> String {
    serde_json::to_string(&response).unwrap_or_else(|_| {
        "{\"schema_version\":2,\"control_plane_schema_version\":1,\"state_schema_version\":2,\"minimum_backend_version\":\"0.1.0\",\"minimum_frontend_version\":\"0.1.0\",\"ok\":false,\"error\":{\"code\":\"encode_error\",\"message\":\"failed to encode response\"}}".to_string()
    })
}

fn cstring(value: String) -> CString {
    CString::new(value).unwrap_or_else(|_| CString::new("{\"schema_version\":2,\"ok\":false,\"error\":{\"code\":\"nul_byte\",\"message\":\"response contained an invalid nul byte\"}}").unwrap())
}

fn sanitize(message: &str) -> String {
    prismux_app::diagnostics::redaction::redact(message)
}

#[cfg(test)]
mod tests {
    use super::*;
    use prismux_core::SaveOptions;
    use std::fs;
    use std::ptr;
    use std::sync::Mutex;
    ...
    fn panic_returns_error_envelope() {
        let payload: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"__panic","payload":{}}"#,
        ))
        .unwrap();
    ...
    fn unsupported_schema_returns_safe_error() {
        let payload: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":3,"op":"dashboard","payload":{}}"#,
        ))
        .unwrap();
    ...
    fn unknown_op_returns_safe_error() {
        let payload: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"missing","payload":{}}"#,
        ))
        .unwrap();
    ...
    fn accounts_returns_versioned_envelope() {
        ...
        unsafe {
            std::env::set_var("PRISMUX_STATE_ROOT", temp.path());
            std::env::set_var("CODEX_HOME", temp.path().join("codex-home"));
        }
        let payload: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"accounts","payload":{"provider":"codex"},"request_id":"r1"}"#,
        ))
        .unwrap();
        unsafe {
            std::env::remove_var("PRISMUX_STATE_ROOT");
    ...
    fn ffi_dashboard_matches_control_plane_for_same_state_root() {
        ...
        let plugins = default_plugins();
        let direct =
            prismux_app::dashboard_view(&plugins, prismux_app::DashboardQuery::default(), None)
                .unwrap();
        ...
        // schema version and on the full aggregate projection. The aggregate
        // carries no per-call timestamp, so it is safe to compare by value; the
        // top-level report's generated_at_unix is not.
        assert_eq!(
            ffi["data"]["control_plane_schema_version"],
            prismux_app::compatibility::CONTROL_PLANE_SCHEMA_VERSION
        );
    ...
    fn dashboard_falls_back_to_last_good_snapshot() {
        ...
        let live: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"dashboard","payload":{"provider":"codex"}}"#,
        ))
        .unwrap();
        let fallback: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"dashboard","payload":{"provider":"missing"}}"#,
        ))
        .unwrap();
    ...
    fn ffi_success_response_can_be_freed_once() {
        ...
        let request =
            CString::new(r#"{"schema_version":2,"op":"accounts","payload":{"provider":"codex"}}"#)
                .unwrap();
        let raw = prismux_menubar_call(request.as_ptr());
        let payload: Value =
    ...
    fn request_contract_ignores_additive_optional_fields() {
        ...
        let payload: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"accounts","payload":{"provider":"codex","future_optional":"ignored"},"future_envelope_field":true}"#,
        ))
        .unwrap();
        unsafe {
            std::env::remove_var("PRISMUX_STATE_ROOT");
    ...
    fn application_error_is_redacted() {
        let payload: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"switch","payload":{"provider":"access_token-secret","local_id":"missing"}}"#,
        ))
        .unwrap();
    ...
    fn accounts_dashboard_switch_and_refresh_match_golden_fixtures() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|err| err.into_inner());
        prismux_app::reset_refresh_state_for_tests();
        let temp = tempfile::tempdir().unwrap();
        unsafe {
            std::env::set_var("PRISMUX_STATE_ROOT", temp.path());
            std::env::set_var("CODEX_HOME", temp.path().join("codex-home"));
        ...
        let cases = [
            (
                r#"{"schema_version":2,"op":"accounts","payload":{"provider":"codex"},"request_id":"accounts-fixture"}"#,
                include_str!("../fixtures/menubar/accounts.response.json"),
                "fixtures/menubar/accounts.response.json",
            ),
            (
                r#"{"schema_version":2,"op":"dashboard","payload":{"provider":"codex"},"request_id":"dashboard-fixture"}"#,
                include_str!("../fixtures/menubar/dashboard.response.json"),
                "fixtures/menubar/dashboard.response.json",
            ),
            (
                r#"{"schema_version":2,"op":"switch","payload":{"provider":"codex","local_id":"missing-local-id"},"request_id":"switch-fixture"}"#,
                include_str!("../fixtures/menubar/switch.response.json"),
                "fixtures/menubar/switch.response.json",
            ),
            (
                r#"{"schema_version":2,"op":"refresh","payload":{"provider":"codex","kind":"interactive"},"request_id":"refresh-fixture"}"#,
    ...
    fn ffi_uses_temp_state_and_codex_home_for_accounts_dashboard_refresh_and_switch() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|err| err.into_inner());
        let temp = tempfile::tempdir().unwrap();
        let state_root = temp.path().join("prismux-state");
        let codex_home = temp.path().join("codex-home");
        fs::create_dir_all(&codex_home).unwrap();

        unsafe {
            std::env::set_var("PRISMUX_STATE_ROOT", &state_root);
        ...
        let accounts: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"accounts","payload":{"provider":"codex"}}"#,
        ))
        .unwrap();
        ...
        let refresh: Value = serde_json::from_str(&call_json(
            r#"{"schema_version":2,"op":"refresh","payload":{"provider":"codex","kind":"interactive"}}"#,
        ))
        .unwrap();
