...
fn imports_lists_and_uses_claude_profile_without_leaking_registry_secret() {
    ...
    let state_db = fs::read(state_root.join("prismux.sqlite")).unwrap();
...
fn duplicate_claude_profile_updates_existing_number() {
    ...
    let plugin = ClaudePlugin::with_paths(temp.join("claude-home"), temp.join("prismux-state"));
...
#[test]
fn claude_profile_uses_explicit_prismux_profile_name_without_storing_it_as_env() {
    ...
    let imported = plugin
        .import_config(ImportConfigOptions {
            name: None,
            content: "PRISMUX_PROFILE=gateway-explicit ANTHROPIC_API_KEY=sk-test".to_string(),
        })
        .unwrap();
    ...
    assert!(!snapshot.contains("PRISMUX_PROFILE"));
