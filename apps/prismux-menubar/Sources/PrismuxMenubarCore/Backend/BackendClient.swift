...
enum Payload: Encodable, Sendable {
    ...
    case login(provider: String, alias: String?, activate: Bool, deviceAuth: Bool)
    case saveExistingLogin(provider: String, alias: String?)
    case importProfile(provider: String, name: String?, content: String)
    case cancelLogin

    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        switch self {
        case .dashboard(let provider):
        ...
        case .login(let provider, let alias, let activate, let deviceAuth):
            try container.encode(provider, forKey: .provider)
            try container.encodeIfPresent(alias, forKey: .alias)
            try container.encode(activate, forKey: .activate)
            try container.encode(deviceAuth, forKey: .deviceAuth)
        case .saveExistingLogin(let provider, let alias):
            try container.encode(provider, forKey: .provider)
            try container.encodeIfPresent(alias, forKey: .alias)
        case .importProfile(let provider, let name, let content):
            try container.encode(provider, forKey: .provider)
...
@_silgen_name("prismux_menubar_call")
private func prismux_menubar_call(_ requestJson: UnsafePointer<CChar>) -> UnsafeMutablePointer<CChar>?

@_silgen_name("prismux_menubar_free")
private func prismux_menubar_free(_ value: UnsafeMutablePointer<CChar>?)
...
public struct RustBackendClient: BackendClient {
    ...
    func call(_ request: BackendRequest) async throws -> BackendEnvelope {
        try await Task.detached(priority: .userInitiated) {
            ...
            guard let raw = requestJson.withCString({ prismux_menubar_call($0) }) else {
