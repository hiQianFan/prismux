...
`display_number` 可保存在 SQLite 中作为 CLI selector/display number，但不得作为 quota/usage 的长期身份。`local_id` 一旦创建不得复用；重复导入同一 auth/profile 时复用原 `local_id`。删除后重新导入同一 auth/profile 可以复活原 archived 记录。
...
OpenMux 需要提供 remove 能力，否则过时账号或 profile 会长期污染 `list/use/refresh`。默认行为应是安全软删除：

- `omx remove <platform> <selector>` 解析 account/profile selector。
- account remove：删除 OpenMux 管理的 auth-bearing snapshot 文件，清除 active target，设置 `accounts.archived_at_unix`。
- profile remove：删除 OpenMux 管理的 profile secret/config snapshot，清除 active target，设置 `profiles.archived_at_unix`。
- archived account/profile 不再参与默认 `list`、`use`、`refresh`、menubar 自动选择。
- 历史 `quota_snapshots`、`refresh_attempts` 和 `usage_events` 默认保留，继续用于历史统计和审计，但展示时标记为 archived。
- 如果当前 active target 被 remove，OpenMux 不自动切到另一个账号，除非用户显式 `omx use`；这样避免误切账号。

第一版不实现复杂 purge/retention。真正需要“忘记所有历史”时再单独设计 `purge`，并且必须明确处理 secret 文件、SQLite 历史和备份文件。

本变更直接落地 SQLite archive 语义，不保留 registry hard-remove 过渡路径。
- 默认 `list/use/refresh` 不再看到该目标。

这满足“过时账号/profile 不再可用”的 CLI 行为，但不保留 archived 历史归属。等 SQLite 状态库落地后，再把 remove 语义升级为 archived row + retained history。
...
- 误删当前账号：remove 前展示将删除的账号/profile safe metadata；默认删除 secret snapshot 但保留 archived 历史；active 被删除后不自动切换。
