...
该 runtime scope 不是系统 active `CODEX_HOME`，也不复制完整用户配置。首次刷新某个已保存 account 时，Prismux 会从该 account 的私有 auth snapshot lazy migration 出最小 `auth.json`，校验 snapshot hash 后写入 managed runtime scope。managed runtime auth 是该 account 的“活凭证”：token 续期只发生在这个 scope 内，不回写系统 active `auth.json`，也不覆盖原始 import snapshot——snapshot 是不可变的身份基线，其 `auth_hash` 就是 account 身份指纹。

每次 quota/status refresh 前，`ensure_fresh_auth` 会保证 managed runtime auth 持有未过期的 access token（按 account `local_id` 串行加锁，避免并发刷新导致 refresh_token 轮换冲突）：

1. runtime access token 的 JWT `exp` 距当前还多于安全余量（120s）→ 直接使用。
2. runtime token 已过期，但 import snapshot 里的 token 更新且仍有效 → 采纳 snapshot 到 runtime（不联网、不消耗 refresh_token）。
3. 两者都过期 → 用最新一份 `refresh_token` 走 OAuth refresh grant（`POST https://auth.openai.com/oauth/token`，`grant_type=refresh_token`，client_id `app_EMoamEEZ73f0CkXaXp7hrann`，复用与 usage 相同的 proxy-aware curl 通道）换新 access/id/refresh token，原子写回 runtime scope；轮换后的 refresh_token 同样只落在 runtime scope。

usage 请求若因 token 被拒返回 401/403，会强制刷新一次再重试；仍失败或 refresh_token 已失效（长期闲置）时，降级为 last-good cached snapshot，并在对应账号卡上给出 auth 类诊断提示重新登录，而不是静默停留在旧数据。用户显式 `prismux use codex <selector>` 或 Menubar activation 时，才走 active auth replacement/promote 路径。
...
Refresh 是 control-plane 的读/更新操作（`prismux-app` runtime），分两种 kind：

- **Interactive**：用户显式触发（Menubar 刷新按钮、账号行菜单、`prismux refresh`）。
- **Background**：Menubar 定时器按 `refresh_cadence_seconds`（默认 300s）触发，以及 popover/进程启动时的一次预热。

per-provider 的 in-memory refresh state 维护 `generation`、`in_flight`、`last_success`、`last_error`：

- **generation + in_flight**：同一 provider 同一时刻只允许一个 refresh 在途；过期 generation 的响应被丢弃，避免旧刷新覆盖新状态。
- **background floor（300s）**：距上次成功不足 300s 的 background 刷新被跳过（`fresh_enough`），避免定时器空转反复打 provider。
- **error backoff（120s）**：background 刷新在一次失败后 120s 内被跳过（`error_backoff`）。

**显式 interactive 刷新不受 floor/backoff 约束**——用户明确要新数据时只受 in_flight 并发保护，闸门只拦机器不拦人。usage 层的失败（如 token 过期、网络错误）走 last-good cached snapshot 兜底并在账号卡上给诊断，不把 provider 拉进 backoff；backoff 只针对 provider 级刷新调用本身返回的错误（如账号列举失败）。

一次 provider 级 refresh（`local_id=None`）会遍历刷新该 provider 的全部账号；account 级 refresh（带 `local_id`）只刷单个。因此 Menubar 一次 background tick 覆盖所有已启用 provider 的每个账号，配合上面 Codex 的 token 续期，常驻运行时每个账号的凭证都会被周期性续上，不会 round-robin 漏刷。需要注意：background floor 与默认 cadence 都是 300s，定时器往往每隔一 tick 才实际执行一次，有效刷新间隔约为 2×cadence；这对 token 新鲜度（access token 寿命以天计）绰绰有余。OpenAI 的 refresh_token 约 14–30 天不活跃即失效，因此长期闲置、且期间 Menubar 从未刷到的账号仍需重新登录。
